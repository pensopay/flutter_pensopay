//
//  PPCreatePaymentSessionRequest.swift
//  PensopaySDK
//
...
public class PPCreatePaymentSessionRequest: PPRequest {
    
    // MARK: - Properties
    
    var id: Int
    var parameters: PPCreatePaymentSessionParameters

    
    // MARK: Init
    
    public init(id: Int, parameters: PPCreatePaymentSessionParameters) {
        self.id = id;
        ...
        super.sendRequest(request: request, success: success) { (data, response, error) in
            if let httpUrlResponse = response as? HTTPURLResponse {
                if httpUrlResponse.statusCode == PensopayHttpStatusCodes.unauthorized.rawValue {
                    Pensopay.logDelegate?.log("The API key needs permissions for 'POST /payments/:id/session'")
                }
            }
...
public class PPCreatePaymentSessionParameters: Encodable {
