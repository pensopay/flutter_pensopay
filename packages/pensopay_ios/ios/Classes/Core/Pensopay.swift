...
public class Pensopay: NSObject {
    ...
    static private(set) var apiKey: String? {
        get {
            if _apiKey == nil {
                logDelegate?.log("\nPensoPay SDK has not been initialized, please do so before usage.\nPensoPay.initWith(api_key: \"<API_KEY>\")\n")
            }
            return _apiKey
        }
        set {
    ...
    public static func initWith(api_key: String) {
        self.apiKey = api_key
        //fetchAquires()
    }
    ...
    public static func openPaymentLink(paymentUrl: String, onCancel: @escaping () -> Void, onResponse: @escaping (Bool) -> Void, presentation: Presentation) {
        OperationQueue.main.addOperation {
            let paymentController = PPPaymentWindowController(paymentUrl: paymentUrl)
            
            let delegate = PPPaymentWindowControllerDelegateCallbacksWrapper()
